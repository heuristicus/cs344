                                ...
                                const float* const h_filter, const size_t filterWidth)
{
  ...
  int filter_memsize = filterWidth * filterWidth;
  checkCudaErrors(cudaMalloc(&d_filter, sizeof(unsigned char) * filter_memsize));
  //TODO:
  //Copy the filter on the host (h_filter) to the memory you just allocated
  //on the GPU.  cudaMemcpy(dst, src, numBytes, cudaMemcpyHostToDevice);
  //Remember to use checkCudaErrors!
  checkCudaErrors(cudaMemcpy(d_filter, h_filter,
                             sizeof(unsigned char) * filter_memsize,
                        ...
                        const int filterWidth)
{
  ...
  //from the image size and and block size.
		const dim3 blockSize(25,25,1);
  const dim3 gridSize(ceil((float)numCols/blockSize.x),ceil((float)numRows/blockSize.x),1);
  printf("block is (%d, %d), gridsize is (%d,%d), image size is (%d, %d)\n",
				 blockSize.x, blockSize.y, gridSize.x, gridSize.y, numCols, numRows);
